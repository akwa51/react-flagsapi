...
const Country = () => {
    ...
    const filterCountries=countriesData.filter(country=>country.name===countryName)
    ...
    useEffect(()=>{
        ...
        if (filterCountries.length>0){
            ...
            for (let i=0;i<arrCountryBorders.length;i++){
                let borderName=countriesData.filter(country=>country.alpha3Code===arrCountryBorders[i])[0].name
  ...
  return (
        <>
            ...
            {!isLoading&&<Link to='/' className='backbtn'>&larr;  Back</Link> }
                ...
                <div className='mainCountryContainer'>
                  {!error && <section className='countryHeadSection'>
                        {filterCountries&&filterCountries.map((item)=>{
                            ...
                            return(
                                <article  key={numericCode} className='countryDetailsContainer'>
                                    <div className='countryflag'>
                                        <img className='country_img' src={flag.toString()} alt={nameOfCountry.toString()}/> 
                                    </div>
                                    <div className='country_group2'>
                                        ...
                                        <div className='county_specs'>
                                            ...
                                            <div className='country_bloc2'>
                                                    ...
                                                    <p className='country_lang'>Languages:  <span  className='language'>{languages&&languages.map((item,index)=>`${index!==languages.length-1?item.name+', ':item.name}`)}</span></p>
                                             </div>                                                    
                                        </div>
